...
class ImageToPdfPreviewScreen extends StatefulWidget {
  ...
  @override
  State<ImageToPdfPreviewScreen> createState() => _ImageToPdfPreviewScreenState();
}
...
class _ImageToPdfPreviewScreenState extends State<ImageToPdfPreviewScreen> {
  ...
  Future<void> _addFromCamera() async {
    ...
    setState(() => _pages.addAll(captured));
  }
  ...
  Future<void> _convert() async {
    ...
    try {
      final bytes = await PdfService.buildPdf(PdfBuildRequest(imagePaths: _pages));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('${_pages.length} page${_pages.length == 1 ? '' : 's'} selected'),
        actions: [
          IconButton(
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('Creating PDF…', style: TextStyle(color: Colors.white)),
                ],
              ),
            ),
        ],
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: ElevatedButton(
            ...
            child: Text(_pages.length > 1 ? 'Convert ${_pages.length} Pages to PDF' : 'Convert to PDF'),
          ),
        ),
      ),
    );
            ...
            child: Row(
              children: [
                ...
                ClipRRect(
                  ...
                  child: Image.file(File(path), width: 52, height: 68, fit: BoxFit.cover),
                ),
                const Spacer(),
                Icon(Icons.drag_handle_rounded, color: colors.descriptionColor),
                IconButton(
                  icon: Icon(Icons.close_rounded, color: colors.descriptionColor),
